  ...
  return (
    <div>
      ...
      <div className=' flex flex-col sm:flex-row items-start gap-5 mt-5'>
        <button className={`py-1 px-3 border rounded text-sm transition-all sm:hidden ${showFliter ? 'bg-primary text-white' : ''}`} onClick={()=> setShowFliter(prev => !prev)}>Filters</button>
        <div className={`flex-col gap-4 text-sm text-gray-600 ${showFliter ? 'flex' : 'hidden sm:flex'}`}>
          ...
          <p onClick={() => speciality === 'Gastroenterologist' ? navigate('/doctors') : navigate('/doctors/Gastroenterologist')} className={`w-[94vw] sm:w-auto pl-3 py-1.5 pr-16 border border-gray-300 rounded transition-all cursor-pointer hover:bg-gray-100 ${speciality === "" ? "bg-indigo-100 text-black" : ""}`}>Gastroenterologist</p>
        </div>
